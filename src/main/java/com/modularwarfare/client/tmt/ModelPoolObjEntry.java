...
public class ModelPoolObjEntry extends ModelPoolEntry {
	...
	@Override
	public void getModel(File file, String objGroup) {
		try {
			...
			if(objGroup != null) {
				ModularWarfare.LOGGER.info("MODULARWARFARE: Now want "+objGroup);
				while ((s = in.readLine()) != null) {
					if (s.startsWith("o ")) {
						if (s.split(" ")[1].equalsIgnoreCase(objGroup)) {
							ModularWarfare.LOGGER.info("MODULARWARFARE: Detected Object: " + s);
							break;
						}
					}
				}
			...
			while ((s = in.readLine()) != null) {
				...
				if(objGroup != null) {
					if (s.startsWith("o ") && !(s.split(" ")[1].equalsIgnoreCase(objGroup))) {
						ModularWarfare.LOGGER.info("MODULARWARFARE: Detected End Object: " + s);
						break;
					}
				}
