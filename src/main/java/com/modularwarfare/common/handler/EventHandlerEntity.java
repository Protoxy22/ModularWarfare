...
import com.modularwarfare.api.cap.BaublesContainerProvider;
...
public class EventHandlerEntity {
	...
	public void attachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof EntityPlayer) {
			event.addCapability(new ResourceLocation(ModularWarfare.MOD_ID, "container"),
	...
	public void playerTick(TickEvent.PlayerTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END) {
			...
			for (int i = 0; i < baubles.getSlots(); i++) {
				...
				IArmor bauble = stack.getCapability(BaublesCapabilities.CAPABILITY_ITEM_BAUBLE, null);
				if (bauble != null) {
